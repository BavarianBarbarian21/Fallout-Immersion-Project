...
public sealed class WorldGenStep_ArktosUrban : WorldGenStep
{
    ...
    public override void GenerateFresh(string seed, PlanetLayer layer)
    {
        ...
        var craterBiome = DefDatabase<BiomeDef>.GetNamedSilentFail("Arktos_Crater");
        var ruinBiome = DefDatabase<BiomeDef>.GetNamedSilentFail("Arktos_Ruin");
        var wastelandBiome = DefDatabase<BiomeDef>.GetNamedSilentFail("Arktos_Wasteland");
    ...
    {
        var creekBiome = DefDatabase<BiomeDef>.GetNamedSilentFail("Arktos_Creek");
        var aqueductBiome = DefDatabase<BiomeDef>.GetNamedSilentFail("Arktos_Aqueduct");
        var floodplainsBiome = DefDatabase<BiomeDef>.GetNamedSilentFail("Arktos_Floodplains");
        var canyonBiome = DefDatabase<BiomeDef>.GetNamedSilentFail("Arktos_Canyon");
        if (creekBiome == null || aqueductBiome == null || floodplainsBiome == null || canyonBiome == null)
        {
            return;
        }
        ...
        foreach (var tile in surfaceTiles)
        {
            ...
            {
                case "Arktos_Preserve":
                    if (RollForTile(seedHash, tileId, 30011) <= 0.5f)
                    {
                        tile.PrimaryBiome = creekBiome;
                    }
                    break;
                case "Arktos_CentralValley":
                case "Arktos_BrahminWoods":
