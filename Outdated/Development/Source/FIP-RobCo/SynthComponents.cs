...
internal static class WestTekSynthGeneUtility
{
    ...
    public static bool IsSynthEndogeneCandidate(Gene gene)
    {
        ...
        return gene.def != RobCoDefOf.RobCo_Gene_SynthComponents
            && gene.def != RobCoDefOf.RobCo_Gene_Courser;
    }
    ...
    private static void ReplaceSynthComponentsWithCourser(Pawn pawn, Gene synthComponentsGene)
    {
        ...
        if (!HasGene(pawn, RobCoDefOf.RobCo_Gene_Courser))
        {
            pawn.genes.AddGene(RobCoDefOf.RobCo_Gene_Courser, xenogene: false);
        }
