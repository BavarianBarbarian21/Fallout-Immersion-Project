# Legacy TranslationSync

This tool is retained only for optional historical regeneration and audit work.
Its Part 1–4 output is written below `.work/legacy-translation-parts` and is
never part of the active `New-Mods` release set. The active translations are
the independent Chinese, Japanese, Korean, and Russian modules.
...
17. Progress and per-mod reports are written below the configured `paths.stateRoot`, never into a distributable mod folder.
...
- When `translation.provider` is enabled, non-English locale text is machine-translated through the configured backend with a cache below `.work/legacy-translation-sync/`.
...
- `paths.repoRoot`: points to the isolated `New-Mods` working set, so historical root mods and Big MT are not scanned.
- `paths.rimWorldRoot`: local RimWorld installation; intentionally empty in the repository configuration.
...
- `paths.fcpCacheRoot`: local folder where FCP repositories are cloned or updated; intentionally empty in the repository configuration.
...
- The four generated legacy outputs are kept under `.work/legacy-translation-parts/FIP-Translation Part 1` through `Part 4`; they are not active mods.
