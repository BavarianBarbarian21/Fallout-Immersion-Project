...
function Get-CategorySearchRoots {
    ...
    switch ($CategoryConfig.id) {
        ...
        'fcp' {
            ...
            if (-not [string]::IsNullOrWhiteSpace($Config.paths.FcpCacheRoot)) {
                $roots.Add($Config.paths.FcpCacheRoot) | Out-Null
            }
            if (-not [string]::IsNullOrWhiteSpace($Config.paths.RimWorldRoot)) {
...
function Sync-Category {
    ...
    $categoryStateRoot = Join-Path $Config.paths.StateRoot $CategoryConfig.id
    $progressPath = Join-Path $categoryStateRoot 'current-sync-progress.txt'
    $startedAt = Get-Date
    $completedOutputs = 0
    $lastCompletedOutput = ''
    ...
    $reportPath = Join-Path $categoryStateRoot 'last-sync-report.txt'
    $summaryLines = @(
